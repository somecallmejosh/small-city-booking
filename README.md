...
- Coverage threshold is 95% (currently ~95.8%).
...
6. `Slot.available` lazily cleans up expired holds so the calendar stays accurate at all times. `HoldExpiryJob` still exists for explicit cleanup but is no longer scheduled.
7. `BookingCompletionJob` runs hourly (Solid Queue) and transitions confirmed bookings to `completed` once all their slots have ended.
...
**Navigation** — The site header is a `HeaderComponent` (ViewComponent) with `renders_one :desktop_nav` and `renders_one :mobile_nav` slots. A `navigation_controller.js` Stimulus controller handles the mobile slide-in drawer with focus-trap and Escape key support.
...
Deployed on [Railway](https://railway.app) with a managed PostgreSQL add-on using the Nixpacks builder. `db:prepare` runs automatically as a pre-deploy command, and the app starts with `rails server -b 0.0.0.0`. A `/up` health check confirms readiness before traffic is routed.
